...
def chat(user_input, hist):
    ...
    for item in hist:
        if item['role'] == 'user':
            langcahin_history.append(HumanMessage(content=item['content']))
        elif item['role'] == 'assistant':
            langcahin_history.append(AIMessage(content=item['content']))

    response = chain.invoke({"input": user_input,"history":langcahin_history})
...
with page:
    ...
    chatbot = gr.Chatbot(avatar_images=[None, 'img.png'],
                         show_label=False,
                         )
